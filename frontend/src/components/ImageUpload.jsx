    ...
    <div className='grid gap-2'>
      ...
      {/* preview */}
      {
        productData.productImg.length>0 &&(
          <div className='grid grid-cols-2 gap-4 mt-3 smLgrid-cols-3'>
            { 
                productData.productImg.map((file,idx)=>{
                    ...
                    return (
                      <Card key={idx} className="relative group overflow-hidden">
                        <CardContent>
                          <img src={preview} alt="" width={200} height={200} className='w-full h-32 object-cover rounded-md'></img>
                          <button onClick={()=>removeImage(idx)} className='absolute top-1 right-1 bg-black/50 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition'><X size={14}></X></button>
                        </CardContent>
                      </Card>
                    )
                })
